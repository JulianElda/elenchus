...
test("show statistics", async () => {
  ...
  await waitFor(async () => {
    ...
    expect(drcount).toHaveValue(
      mock_user_admin.statisticsUserBox.countDatarooms
    );
  });

  await waitFor(async () => {
    const creationDate = screen.getByLabelText(/Creation date/i);
    expect(creationDate).toHaveValue(
      formatDate(mock_user_admin.userCreation.registrationDate)
    );
  });

  await waitFor(async () => {
    const bookedAccount = screen.getByLabelText(/Booked account/i);
    expect(bookedAccount).toHaveValue(
      formatSize(mock_user_admin.statisticsUserStorage?.bookedAccountStorage)
    );
  });
});
